...
Satt opp for **Hustadvika vidaregåande skole**: nynorsk, klassekoder og fag fra
videregående, og skolens egen profilfarge og logo. Alt sammen står i Oppsett og
kan byttes.
...
Her står også de tre feltene som gjør planen til skolens egen:

| Felt | Hva det gjør |
| --- | --- |
| Språk | `bokmål` eller `nynorsk`. Styrer både arkene og nettsiden. |
| Profilfarge | Hex-koden til skolens farge, f.eks. `#0093C9`. Fargen brukes på «i dag», fristmerkene og klassen i overskriften – og lysnes automatisk i mørk modus. |
...
- **Lys eller mørk** følger telefonen, men knappen overstyrer og valget huskes.
  Logoen får hvit plate i mørk modus, så den holder seg lesbar.
...
| `ukeplan.py ny --skole "Hustadvika vidaregåande skole" --uke 41` | med skolenavn og uke satt |
